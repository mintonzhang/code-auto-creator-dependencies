...
public class ZipUtil {
     ...
     * 压缩一个或多个文件
     */
    private static void zipFiles(Collection<File> srcFiles, ZipOutputStream zipOut) throws IOException {
     ...
     * 压缩文件夹
     */
    public static void zipDictionary(File fileDir, String fileName, ZipOutputStream zipOut) throws IOException {
        if (fileDir.isHidden()) {
            return;
        }
        if (fileDir.isDirectory()) {
            ...
            if (children != null) {
                for (File childFile : children) {
                    zipDictionary(childFile, fileName + "/" + childFile.getName(), zipOut);
                }
            }
            return;
        }
    ...
    public static void unZip(File zipFile, File saveDir, boolean ifErrorContinue) throws IOException {
